# ==========================================================
# pba_ts_multi.py  — PBA-TS (path-loss/SINR/QoS/最悪干渉) 多エージェント完全版
# 依存: numpy, matplotlib
# 実行例:
...
def run_sim(cfg: Config, verbose: bool = True):
    """
    多エージェント版 PBA-TS（論文 Algorithm 3.2 に対応）
      - SU は ω = 0..S-1 台すべてが学習
      - 各 SU は Q 本のチャネルと L 個の電力レベルを腕として持つ
        => 腕 = (ω, q, ℓ) （ℓ は電力レベルのインデックス）
      - 各スロット r で、全 SU が同時に PBA-TS を実行
      - 干渉計算には「全 SU の選んだ電力 p_{ω,q}(r)」を使用
      - 返り値 avg_sum_rate は「システム全体の平均サムレート [bit/slot]」
    """
    ...
    # 離散電力レベル集合 P_set: 1/L, 2/L, ..., 1 × Pmax
    L = cfg.power_levels
    ...
    if verbose:
        print(f"[INFO] delta (30 dB SNR w/o interference) ≈ {delta_m:.2f} m")
    ...
    # ==== 経路利得 ====
    # SU 自己リンク (ω→ω)
    d_su_su = distances(su_tx, su_rx)                  # (S,S)
    g_su_su = path_gain(cfg, np.diag(d_su_su), fq_Hz)  # (S,)

    # SU ↔ SU 交差リンク (ω'→ω)
    g_su_to_su = path_gain(cfg, d_su_su, fq_Hz)        # (S,S)

    # SU → PU_rx (ω→PU_q_rx)
    d_su_purx = distances(su_tx, pu_rx)                # (S,Q)
    g_su_purx = path_gain(cfg, d_su_purx, fq_Hz)       # (S,Q)

    # PU_q Tx → PU_q Rx
    d_pu_self = np.diag(distances(pu_tx, pu_rx))       # (Q,)
    ...
    # ==== TS 統計（腕 = (ω,q,ℓ)）====
    # 形状はすべて (S, Q, L)
    T      = np.zeros((S, Q, L), dtype=int)    # T_ω,q^(p)
    mu     = np.zeros((S, Q, L), dtype=float)  # µ̂_ω,q^(p)
    sigma2 = np.ones((S, Q, L), dtype=float)   # σ²^(p) 初期値 = 1

    regret = np.zeros(R, dtype=float)
    total_sum_rate = 0.0  # システム全体のスループット累積 [bit]

    Ppu_W = dbm_to_w(cfg.P_pu_tx_dbm)

    # ==== Oracle（近似上限）====
    # 「他の SU は送信しない」理想状態で，各 (ω,q) が取り得る最大レートを足し合わせたもの
    oracle_best = 0.0
    for w in range(S):
        for q in range(Q):
            best_r_wq = 0.0
            for p in P_set:
                # SU-SINR（SU_ω のみ送信，PU からの干渉のみ）
                ...
                if (gamma_su >= gTH_su) and (gamma_pu >= gTH_pu):
                    r_wq = achievable_rate(cfg, gamma_su)
                    if r_wq > best_r_wq:
                        best_r_wq = r_wq
            oracle_best += best_r_wq
    if verbose:
        print(f"[INFO] Approx. oracle best sum rate: {oracle_best / 1e6:.3f} Mbit/slot")

    # ==== スロットループ ====
    if verbose:
        print("[INFO] Multi-agent PBA-TS simulation start...")

    for t in range(R):
        # 各 SU の残余電力 p_{ω,M}(t) を初期化（論文の p_{ω,M}）
        p_residual = np.full(S, Pmax_W, dtype=float)

        # 本スロットでの SU 送信電力行列 p_su[ω,q]
        p_su = np.zeros((S, Q), dtype=float)
        # どの電力レベル ℓ を選んだかを記録 (更新用)
        chosen_idx = -np.ones((S, Q), dtype=int)

        slot_rate = 0.0  # このスロットのサムレート

        # ---- サブバンド q を順番に処理 (Algorithm 3.2 内側ループ) ----
        for q in range(Q):
            # 各 SU についてこのサブバンド q の電力を決定
            for w in range(S):
                if p_residual[w] <= 0.0:
                    continue

                # 残余電力以下の候補だけを考慮
                feasible_idx = np.where(P_set <= p_residual[w])[0]
                if feasible_idx.size == 0:
                    continue

                # TS: 各電力レベルごとに φ をサンプリング
                mu_f  = mu[w, q, feasible_idx]
                var_f = np.maximum(sigma2[w, q, feasible_idx], 1e-12)
                phi_f = rng.normal(loc=mu_f, scale=np.sqrt(var_f))   # 形状: (len(feasible_idx),)

                # Algorithm 3.2: argmax_p [ φ_{ω,q}^{(p)} - p / p_{ω,M} ]
                scores = phi_f - (P_set[feasible_idx] / p_residual[w])
                best_pos = int(np.argmax(scores))
                l_star   = int(feasible_idx[best_pos])

                p = float(P_set[l_star])
                p_su[w, q]       = p
                chosen_idx[w, q] = l_star

            # ---- このサブバンド q での SINR を一括計算 ----
            # PU_q の SINR
            ...
            # 各 SU の SINR と報酬更新
            for w in range(S):
                l_star = chosen_idx[w, q]
                if l_star < 0:
                    continue  # そもそも送信していない

                p = p_su[w, q]
                if p <= 0.0:
                    continue

                # SU_ω への他SU干渉: Σ_{w'≠w} p_{w',q} * g_{w'→w}
                I_su_wq = float(np.sum(p_su[:, q] * g_su_to_su[:, w]) - p * g_su_to_su[w, w])
                ...
                # 成功時: R > 0、失敗時: R = 0 （論文どおり）
                if success:
                    r_wq = achievable_rate(cfg, gamma_su_wq)
                    slot_rate      += r_wq
                    total_sum_rate += r_wq
                else:
                    r_wq = 0.0

                # 成功・失敗に関わらず「送った」という事実で統計更新（TS） & 残余電力消費
                T[w, q, l_star] += 1
                t_cnt = T[w, q, l_star]
                mu[w, q, l_star] += (r_wq - mu[w, q, l_star]) / t_cnt
                sigma2[w, q, l_star] = 1.0 / (t_cnt + 1.0)

                # 残余電力を消費（論文の p_{ω,M} 更新）
                p_residual[w] = max(p_residual[w] - p, 0.0)

        # ---- Regret 更新（システム全体のサムレート vs oracle）----
        if t == 0:
        ...
        if verbose and (t % cfg.print_every == 0):
            print(
                f"[TS] t={t:5d}  "
                f"sum_rate={slot_rate/1e6:.3f} Mbit  "
                f"norm={slot_rate/cfg.W_Hz:.3f} bps/Hz  "
                f"regret={regret[t]/1e6:.3f} Mbit"
            )

    # ==== 結果まとめ ====
    avg_sum_rate = total_sum_rate / R  # [bit/slot] システム全体
    if verbose:
        print("[INFO] Simulation done.\n")
        print("========== Multi-agent PBA-TS summary ==========")
        print(f"平均サムレート: {avg_sum_rate/1e6:.3f} Mbit/slot")
        print(f"(正規化すると: {avg_sum_rate/cfg.W_Hz:.3f} bps/Hz)")
        print("===============================================")
    ...
    if cfg.plot:
        ...
        plt.title("Multi-agent PBA-TS regret (approx.)")
        plt.tight_layout()
        plt.show()

        # レイアウト図
        plot_layout(
            su_tx, su_rx, pu_tx, pu_rx,
            area_m=int(cfg.area_km * 1000),
            save="layout.png", show=True
        )
